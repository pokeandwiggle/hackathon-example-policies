...
def inference_loop(
    ...
    """Run policy inference using observations from dataset on real robot.
    ...
    input("Press Enter to start policy inference with dataset observations...")

    step = 0
    done = False

    # Inference Loop
    print("Starting policy inference loop with dataset observations...")
    period = 1.0 / hz
...
def main():
    parser = argparse.ArgumentParser(
        description="Run policy inference with dataset observations on real robot"
    )
    parser.add_argument(
        "--checkpoint",
        type=Path,
