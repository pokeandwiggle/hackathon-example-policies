...
def inference_loop(
    ...
    """Run policy inference using images from dataset but robot state from real robot.
    ...
    input(
        "Press Enter to start policy inference with hybrid observations (dataset images + robot state)..."
    )
    ...
    while not done:
        ...
        try:
            ...
            print(f"\n[cyan]Step {step}:[/cyan]")
            print(f"  Using images from dataset frame {step}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run policy inference with hybrid observations (dataset images + robot state) on real robot"
    )
    parser.add_argument(
        "--checkpoint",
        type=Path,
